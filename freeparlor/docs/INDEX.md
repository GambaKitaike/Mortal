...
## design/ — 設計・pre-registration（12本）
...
| `reports/early_damage_probe_result_20260729.md` | 2026-07-29 | active | **判定非関与の探索的診断**。step 0–2000 を 100 step 刻みで測る短 run を **replicate 2本**（各 3.1h）+ Arm K step2000 の n=400 再測で、交絡を消した3点比較まで実施。**§9 が最新の結論**: 同一条件で放銃劣化は **+2.04pp(z=+2.70) / +0.12pp(+0.16) / +0.96pp(+1.32)** と連続的に散らばり **どの run も外れ値ではない**＝**run 間ばらつきが効果量と同程度**。一方 **立直シフト（step200 で 63–67%）と降りの中断率の悪化は再現**。含意は「1変数 = 1 run で z≈2 級を判定する」設計の検出力（0b の資源配分）。§0–§8 は初回 run 単独時点の記録として不改変。 |
...
| `ops/policy_session_0b_decisions_20260729.md` | 2026-07-29 | **裁定記録（確定）** | **0b の実際の決定**。進路の正。全実験に掛かる **seed 方針**（1 seed でスクリーニング → 判定を満たしたら 2 seed 目 → 食い違えば「効果不確実」で保留）、次の軸 = **L1 O1→O3 → 候補2 搾取者 → 候補1 oracle 蒸留**、議題1 **否決（条件つき）**、議題2 現行ルールでは行わない、議題4/5 保留。`policy_session_0b_frame.md` は準備資料で決定を含まない。 |
| `ops/session_handover_20260729.md` | 2026-07-29 | active | **最新の申し送り**。洗い出し → 負債返済 → 指標実装 → 初期損傷プローブ（replicate 2本）→ kl_beta=0.4 再走の発進まで。**最重要は「同一 config の run 間ばらつきが効果量と同程度」**（§2a）と「現行 eval SE は保守的」（§2b）。走行中 run の完走後手順は §3、タスク一覧と優先度は §4、裁定事項は §5。 |
| `ops/session_handover_20260728.md` | 2026-07-28 | active（§1b は 07-29 版で更新） | **次セッション（やるべきことの洗い出しと優先順位付け）のための材料整理**。anchor 系列の完了内容と未決事項。裁定は行わない。**優先順位付けの正はこれ**。 |
