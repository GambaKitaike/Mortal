# Free Parlor Reward — Mortal 報酬設計実験（DQN/CQL 時代の README・アーカイブ）

> **アーカイブ注記（2026-07-25）:** 本文書は 2026-06 時点の `freeparlor/README.md`
> （オフライン DQN + CQL 時代、Phase1〜4）をそのまま保全したもの。
> 記載の実験・結論はいずれも **教師データ（天鳳2009棋譜）依存の CQL 経路**に関するもので、
> 現行の**教師データ非依存・自己対戦 PPO 本線**（2026-07-02 以降）には適用されない。
> 「今後の課題」節の未実装項目は、その後の PPO 移行・探索ラダー Stage1〜3・DRCA プローブで
> 別の形で追跡・決着している。現行の案内は `freeparlor/README.md`、
> 全体像はリポジトリルートの `README.md` を参照。
> 本文は歴史文書として不改変で保全（本注記の追加と、
> `docs/archive/dqn/` 移設に伴う相対リンクの修正のみ）。
...
`α = β = γ = 1` としたとき、実際の金銭精算額と一致する設計（[phase4_chip.md](./phase4_chip.md) に検算あり）。
...
人間水準に近い打牌であることを確認し、以降の比較基準とした。詳細: [phase1_stats_192x40.md](./phase1_stats_192x40.md)
...
**報酬を変えると打牌が変わる**ことを実証。詳細: [phase2_result.md](./phase2_result.md)
...
詳細: [phase3_sweep.md](./phase3_sweep.md)
...
**赤ドラ条件別分析**（[phase4_aka_conditional.md](./phase4_aka_conditional.md)）: 人間の正着「赤持ち → 鳴いて上がってチップ確保」は**いずれの β でも副露率では確認できない**。代わりに「赤持ち → 立直率↑」が一貫。原因は**赤を抱えて流局した取りこぼし損失が報酬に無い**ことと特定。

詳細: [phase4_chip.md](./phase4_chip.md)
...
| ファイル | 内容 |
|---|---|
| [phase1_stats_192x40.md](./phase1_stats_192x40.md) | Phase 1 打牌統計 |
| [phase2_result.md](./phase2_result.md) | Phase 2 報酬差し替え結果 |
| [phase3_sweep.md](./phase3_sweep.md) | Phase 3 α:γ スイープ |
| [phase4_chip.md](./phase4_chip.md) | Phase 4 チップ報酬・β スイープ |
| [phase4_aka_conditional.md](./phase4_aka_conditional.md) | Phase 4 赤ドラ条件別分析 |
