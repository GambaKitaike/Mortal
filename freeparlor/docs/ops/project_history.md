# プロジェクト経緯（CLAUDE.md「現在の状態」節の履歴保全）

**ステータス: 履歴文書（追記のみ）。** 2026-07-25 に `CLAUDE.md` の肥大化した
「現在の状態」節（Stage1 判定〜anchor Arm C 発進まで、約 700 行）を本書へ移設した。
**本文は不改変で保全**しており、各エントリは当時の記述のまま（後から真偽が更新された
記述もそのまま残す — 訂正は後続エントリ側に書かれている）。

- 現在進行中・未決の事項は `CLAUDE.md`「現在の状態」節（移設後は現況のみ）が正。
- 設計・判定の正典は `freeparlor/docs/design/` と `freeparlor/docs/reports/`。
  本書はあくまで「いつ何をして何が起きたか」の時系列記録であり、判定の根拠ではない。
- 索引は `freeparlor/docs/INDEX.md`。
- タスク完了時にこの節を更新する規律は継続する（完了して current でなくなった項目を
  `CLAUDE.md` から本書の末尾へ移す）。

時系列は上から古い順（2026-07-06 Stage1 判定 → 2026-07-25 anchor Arm C 発進）。

---

## 経緯（2026-07-06 〜 2026-07-25）
...
- **DRCA frame1 完走 + 単枠集計 + ΔQ̂極値24件の専門家レビュー記録（判定非関与）**
  （2026-07-21〜22）: 第1枠（セット(a)×Stage1-16000）は 3 shard とも正常完走
  （6960 rollout、illegal_action_fallback=0、divergence 0。7/17 PC 移設は
  stop→tar退避→`--resume` で無事往復）。単枠集計（pilot 50 + main 435 = N=485 結合、
  pilot 旧スキーマは branch_role/seq を bp.jsonl から補完）: **ΔQ̄ = −3.2635 千点、
  cluster SE = 0.7208、|ΔQ̄|/SE = 4.53**（符号検定 219+/255−/11◦ p=0.11 n.s. —
  負け幅が勝ち幅より大きい非対称）。exploratory 層別: chi −5.13 / pon −1.23 /
  kan +1.95（n=19）。結果は `aggregate_final.json`（run dir、判定は全5枠揃い後）。
  極値24件（負12+正12）を対象牌付き機械生成テーブル
  （`frame1_extremes_summary.md`、局/巡/対象牌を牌譜から機械解決）で Gamba が
  全件目視評価 → `qualitative_expert_review_drca_frame1_20260722.md` に記録:
  **負の裾は ~9/12 が本物の手壊しコスト（親の満貫級和了を壊す型が支配的）、
  正の裾は本物4（役牌/祝儀/間に合わせ）+ 結果論5 の混合**。「結果論」の大半は
  パス後に方策が押して放銃する型で、方策の降りの雑さの織り込み = 方策条件付き
  反実仮想の外的妥当性の限界として整理。負側#3（立直宣言牌ポン後の危険牌押しが
  負値の主因）は解釈C のメカニズムの目視実例候補としてセット(b) 読解の参照点に指定。
  **専門家の反論2件を受け同 md §4 に監督側訂正を追記（2026-07-22）**: (a) 分布は
  裾支配（中央値 −0.18、負側極値12件=2.5% が全シグナルの 47.8%。対称トリムでは
  負のまま、負側のみ10%除外で +0.59 反転）→ 主判定の推定対象は「無差別な鳴きの
  平均」であり「選択的な鳴きの価値」ではないと明記（取りこぼし仮説は集計 ΔQ̄<0 と
  両立）、(b) probe は同一山（seed_start=元対局 seed）のため「ツモ番ずれ」型の
  結果論は腕内共通ショックで K=8 では消えない — 監督の「降りの雑さ」説は主説明
  として撤回・降格、セット(b) の事前期待（ΔQ̂ が動かない可能性と第一解釈候補）を
  測定前に記録。
  **第2枠 a_init 発進済み（2026-07-21 19:07、`main_a_init_20260721_184910`、
  採取485点 wall 1073s・shard 162/162/161・3プロセス並走中、発進前に libriichi
  rebuild + verify 全18検定 PASS）**
...
- **anchor 系列 設計凍結 + 実装タスク委任 + 第2枠併合裁定**（2026-07-25 夕、
  Gamba 裁定・docs-only・コード変更なし）: `anchored_ppo_design.md` を凍結
  （当該 commit = 事前登録・以後変更禁止。唯一の許容は §6a 再走規定の機械的適用）。
  確定値: 実施順 C→K、anchor_prob=0.25、kl_beta=0.1（スモーク帯域較正は不採用 —
  step0 で KL=0 のため情報にならない）、判定 = 基礎維持（放銃差 z<2、
  `analyze_fundamentals_1v3.py`）+ チップ +方向≥1SE、1v3 両脚 n=800
  （seeds [10000,10200)）、判定窓 8k–16k、K のみ最大1回の機械的再走（過強→β/4、
  過弱→β×4）、C は1発勝負。GPU 順序: DRCA 第3枠完走 → Arm C 発進が最優先、
  DRCA 残枠（a_s3mid/b_s3final）は後回し。**実装は Composer/Sonnet へ委任** —
  タスクプロンプト `freeparlor/docs/ops/anchor_impl_task_20260725.md`
  （arm C/K 配線・検定(19)(20) は CPU 単体実行可能設計・config/launcher/機械ゲート
...
- **DRCA 第3枠 中断 + 測定打ち切り裁定（§5a-1c）**（2026-07-25、Gamba 裁定・
  GPU 解放）: 第3枠（セット(a)×Stage3-16000）を進捗 4.6%（359/7760 rollout、
  経過 4h）で**中断**。中止ではなく中断で、成果物は
  `backups/drca/main_a_s3final_20260725_120218_20260725_163836.tar.gz`（2.6MB）へ
  退避し `run_resume.sh` で再開可能。残枠（同枠の残り・a_s3mid・b_s3final）は未測定。
  理由: (1) 中核の問い（反鳴き均衡は基礎劣化アーティファクトか経済の性質か）は
  init −2.14/4.46SE と Stage1-16000 −3.26/4.53SE の挟み撃ちで決着済み、(2) Stage3 の
  raw advantage は Stage1 をほぼ定量再現済み（§7a）で第3枠は第1枠の再現になる公算大、
  (3) 優先軸が anchor 系列へ。**放棄する内容は §5a-1c に明記**（主 contrast 2 全損 →
  解釈 C 判定せず、contrast 3 測定不能、§4 の A/B/C/D/N 割り当てに到達しない。
  最終報告では未測定枠の存在を必ず明示 = 選択的報告の回避）。これは削減規則の
  機械適用ではなく新規裁定であり、解釈条件・既測定枠の数値には非干渉
- **インシデント: 孤児化 verify プロセスの検出・除去**（2026-07-25 16:38）: 停止後の
  残党チェックで `verify_ppo_p1.py`（PID 45706、15:54:54 起動・44分経過・CPU時間
  3h18m・親 /init）を検出。fd が cursor-server ログを指しており実装セッション
  （Composer）由来と特定。差し戻しプロンプト §5「verify 全20本はやらない」に反し、
  **DRCA 第3枠と GPU を同時使用していた**（GPU 1系統ルール違反）。第3枠の測定値は
  fork-by-replay の digest 照合により決定論的で divergence 0 のため無効化なし。
...
## 付録: バックログ原文（2026-07-16 起草・2026-07-25 時点の CLAUDE.md より）
...
0. **DRCA プローブ（診断、事前登録済み）**: ~~解釈条件の凍結~~ ~~実装（採取・並走・
   ...
   **48h 条項適用済み（2026-07-16、§5a-1b: セット(b)×init 脱落で実効5枠）+
   ...
   ~~第1枠~~ ~~第2枠 a_init~~ **完走・単枠集計済み（第1枠 −3.26/4.53SE、
   第2枠 −2.14/4.46SE — 「現在の状態」節参照）**。第3枠 a_s3final 走行中
   （~07-29 完走見込み）。残枠 a_s3mid / b_s3final は **anchor 系列より後回し**
   （2026-07-25 凍結裁定、実施可否は改めて裁定）→ 実効枠揃い後に判定。
...
4. ~~launcher Cleanup ステップの修正~~ **実装済み・logic検証済み（2026-07-25、bc80aff、
   run-validation は次回発進に持ち越し）**: `run_ppo_p3_stage1_inner.sh`（全 stage/resume
   発進が exec する共有 inner）の3修正 — (1) trainer_watchdog は exit 0 完走で break
   （二重起動解消）、(2) monitor は watchdog 死亡×steps≥MAX を正常完走 break として扱う
   （回帰防止）、(3) cleanup に run_server.py/run_client.py のパターン pkill 追加
   （conda run が SIGTERM を子 python へ転送せず孤児化していた残党放置の修正）。
...
6. ~~verify 冒頭への .so 鮮度チェック追加~~ **実装済み・GPU非依存検証済み（2026-07-25、
   ...
   .rs が import 済み .so より新しい / target/release の .so が新しい（cp 飛ばし）の
   2シグナルで FATAL SystemExit。2026-07-08 ビルド事故クラスの検出。実ツリー fresh
   PASS + 合成 stale 2経路を独立確認
7. ~~drain 清掃の恒久運用~~ **明文化済み（2026-07-16、
   `freeparlor/docs/ops/run_artifact_retention.md`）+ 清掃候補3件の削除実施済み
   （同日 Gamba 承認、計411GB回収 — 「現在の状態」節参照）**。~~`buffer/` の用途調査~~
   **消化済み（2026-07-24）**: buffer/ = client→server 受領ステージング（drain の前段、
   `server.py` のみが参照・起動時 rmtree）。run 終了後の残量は未 drain・未消費の
   in-flight データ（stage2_resume 36GB は mtime 実証で全量 trainer 終了後の残党生産）。
   `run_artifact_retention.md` の分類表を「drain と同ポリシー」で確定、清掃候補4件
   計 ~43GB を §5 に登録。**削除も実施済み（2026-07-25、Gamba 承認・§2 手順）**:
   許可リスト4パスのみ明示引数で削除、df 85G→43G 使用（42GB 回収・空き 914G）、
   4 run の checkpoints/logs/config 実在確認済み、DRCA 第2枠は前後で稼働継続。
   §5 清掃候補は全件処理済み（DQN 遺産 ~20GB はスコープ外）。**バックログ7 完了**
...
## 付録: anchor 系列の run 経緯（2026-07-31 に CLAUDE.md から不改変で移設）

> 系列は 2026-07-31 に閉幕。総括は `../reports/anchor_series_summary_20260731.md`。
> 以下は CLAUDE.md「現在の状態」節にあった記述をそのまま移したもの。
