...
重要な含意（**2026-07-24 Gamba 第2次補足で訂正**）: 面前祝儀ルールでも**役満は
鳴き和了でもチップ対象**。裏・一発は構造的に立直（=面前）専用なので、
**面前祝儀/鳴き祝儀の軸で実効的に動くのは赤の扱いのみ**（鳴き手の赤にチップを
払うか払わないか）。そして現行 `chip_base` は赤を面前条件なしで払っている（F4）
— つまり**現行は赤について既に「鳴き祝儀」相当**。帰結:
- この軸で鳴き誘因を現行より「足す」余地はない。面前祝儀プリセットは鳴き手の赤
  チップを**取り上げる**方向にのみ動き、機会費用ギャップ（~5.5–5.9 チップ）は
  **拡大方向**（予測: 鳴きはさらに減り立直特化がさらに強まる）。
- 本書旧版の「鳴き祝儀の追加が機会費用ギャップを直接埋める最標的レバー」は**誤りに
  つき撤回**（`policy_session_0b_frame.md` §5.2-4 も同時訂正）。ギャップを経済側から
  縮める標準ルール内のレバーは **chip_value（チップ単価）・赤枚数等の別軸**に限られる。
- 面前/鳴き軸の価値は (a) 実雀荘ルール差の商品再現（プリセットとして必要）と
  (b) **逆方向の対照実験**（面前祝儀化で鳴きがさらに減れば「赤チップが現行の残存
  鳴き誘因である」ことの検証になる）に整理し直す。
...
- **プリセット1つ = 訓練 run 1本**（16k ≈ 2日 GPU + drain ~370GB）。配備税の教訓
  ...
  問題設定であり、0b で併せて裁定。
- 軸の優先順位（暫定、**2026-07-24 第2次補足で更新**）: 旧版の「鳴き祝儀 →
  ウマ表 → チップ単価」は前提誤り（上記 §3.1 訂正）につき撤回。更新後の整理:
  鳴き誘導方向の実験なら **チップ単価/赤枚数**、立直特化の頑健性検証（逆方向対照）
  なら **面前祝儀化**、商品プリセット網羅なら **ウマ表** — どれを先に焼くかは
  実験目的の裁定そのものなので **0b で決める**（本書では優先順位を予断しない）。
