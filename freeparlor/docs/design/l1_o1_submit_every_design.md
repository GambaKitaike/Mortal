...
### 12b. 完走と eval（2026-08-01）

**数値は `freeparlor/docs/reports/l1_o1_result_20260801.md` が正**（判定は未起草。
レンズ4 の後に監督側が行う）。要点のみ:

- 完走: 07-31 01:44 → 08-01 05:21（27.6h・16000 step・launcher `exit=0`・監視6項目ゼロ）
- **判定1（放銃 z<2）: z = +4.91**（+2.635pp）
- **判定2（チップ +方向 ≥1SE）: +0.381 枚/半荘 = +1.62SE**
- 機構: staleness **73.0**（= transit 68.5 + 量子化 4.5）で予測どおり **−21%**。
  一方 **clip@epoch1 は 0.2046 で参照 run（0.185–0.214）の内側 = 動かなかった**
- baseline 比: Δ_O1 − Δ_ref = **−0.241pp（−0.32 z、n.s.）**

**§7 のコマンド例の欠陥（実地で判明・要訂正）**: 参照脚のコマンドに
`GRP_BASELINE_CKPT` が無いため、`eval_grp_baseline_1v3.py` の既定値
（`/home/gamba/mahjong/runs/grp_baseline.pth` = DQN 期の `steps=0` ファイル）に
フォールバックし、初回測定が未学習相手との対戦になった（800半荘中 798勝）。
`GRP_BASELINE_CKPT=$INIT_CKPT` を明示して再測した値が上記。**§7 を使うときは
必ず `GRP_BASELINE_CKPT` を明示すること**（凍結ルールにより §7 本文は改変せず本節に注記）。
判定1/2 の2脚は正規ラッパ経由で正しい init を相手にしており影響なし
（詳細は結果 md §4）。
| 判定1（放銃差 z） | — |
| 判定2（チップ差 /SE） | — |
| `Δ放銃_ref`（n=800 実測） | — |
