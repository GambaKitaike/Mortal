...
### §6a 適用の記録（2026-07-29、機械的適用）

**適用条件の照合**: Arm K は判定1✗（放銃 z=+2.51 ≥ 2、`anchor_arm_k_result.md` §2）。
§6a の「判定1✗（アンカー過弱）→ **kl_beta × 4** で再走」に**機械的に該当**する。
これは新規の設計判断ではなく、凍結済み規定の適用である。

| 項目 | 値 |
|---|---|
| 使用した β | **kl_beta = 0.4**（Arm K の 0.1 × 4） |
| run | `anchor_k_b04_<日時>` |
| config | `freeparlor/configs/ppo_anchor_k_b04.toml`（`ppo_anchor_k.toml` との diff は **run パス + kl_beta のみ**） |
| 判定条件 | **§6 と同一**（放銃差 z<2 / チップ +方向≥1SE / 1v3 両脚 n=800・seeds [10000,10200)） |
| 判定窓 | step 8000–16000（従来踏襲） |
| 再走の可否 | **これが最後**。§6a により、この結果が最終（それ以上の再走禁止） |
| 結果 | **判定1 ✗（放銃 +1.150pp、z=+2.21）/ 判定2 ○（チップ +0.405、+1.75SE）= 象限 III**（Arm K と同じ）。判定は `../reports/anchor_k_b04_result.md`、測定は `../reports/anchor_k_b04_measurements_20260730.md`、レンズ4 は `../reports/qualitative_review_anchor_k_b04_20260731.md` |

**β×4 の効果（記録）**: `kl_ref_mean` 全期間平均は 0.13048（K）→ **0.06895**（b04）と
**約半分**になり、β は機構として効いた。それでも判定は動かなかった
（放銃 z +2.51 → +2.21 / チップ +2.18SE → +1.75SE）。**どちらの変化も
同一 config の run 間ばらつきより小さい**ので「β=0.4 では足りなかった」と読むのは過剰で、
正しくは「**この1本では閾値を跨げなかった**」。

### 系列の閉幕（2026-07-31、Gamba 裁定）

§6a の再走規定は b04 で**使い切った**。3 arm すべてで判定1 が不成立であり、
**anchor 系列はここで閉じる**。系列の総括は **`../reports/anchor_series_summary_20260731.md`**。
本設計書は以後 **closed**（凍結のまま。結果の追記も本節で完了）。

**単一変数を保つため `diag_save_every` は入れない**（観測専用で学習には影響しないが、
§6a は「β の変更は機械的に」と定めており、Arm K との diff を 1 行に保つことを優先した）。

**解釈上の留保（2026-07-29 に判明した事実。判定条件は変更しない）**:
`early_damage_probe_result_20260729.md` §9 が、**同一 config の run 間ばらつきが
効果量と同程度**であることを replicate 2本で示した（step2000 の放銃劣化が
同一条件で z=+2.70 / +0.16 / +1.32 と散らばる）。したがって本再走が
「判定1 の閾値を跨ぐか」を1本で決める検出力は限定的である。
**判定条件は凍結済みなので変更しないが、結果の読みにこの留保を必ず添えること。**
また `session_handover_20260728.md` §2 が §1b を根拠に立てた
「初期 2000 step のジャンプを抑えられるか」という問い方は、
そのジャンプの水準が再現しないことが分かったので**採らない**。
問いは §6 の判定条件そのもの（判定窓での放銃差とチップ）に戻す。
