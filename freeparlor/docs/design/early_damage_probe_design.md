...
### 4b. replicate run の追加（2026-07-29、**発進前に目的を明記**）

初回 run（`early_probe_20260728_202533`）で**陽性対照が失敗した** —
終端の放銃劣化が Arm K の step2000（n=800、z=+3.91）を再現せず z=+0.16 だった
（`../reports/early_damage_probe_result_20260729.md`）。config・相手プール・
KL 乖離量・π(立直) の推移はいずれも一致しており、**同一 config の run 間ばらつき**が
疑われる。

**n=2 では「どちらが典型か」を決める材料が無い**ので、同一 config の replicate を
1本追加する。**目的を発進前に固定しておく**（結果を見てから解釈を作らないため）:

> **この replicate が答える問い**: step2000 近傍の放銃劣化は、同一 config の run 間で
> どれだけばらつくか。具体的には 3 run（Arm K の step2000、初回プローブの step1900、
> 本 replicate の step1900）の z が
> **(a) 揃う（初回が外れ値）/ (b) 散らばる（この量は run 間ばらつきが支配的）**
> のどちらの像を示すか。

**判定条件は置かない**（本設計は元から診断・判定非関与）。n=3 でも分散の点推定しか
得られないので、**「ばらつきが大きい/小さい」の定量的な結論は出さず、
3点の実測を並べて示すにとどめる**。

構成は初回と完全に同一（`kl_beta=0.1` / `max_steps=2000` / `diag_save_every=100` /
`save_every=2000`）。変えるのは run dir だけ。eval も同一（10 ckpt × n=400、
seeds [10000,10100)、終端 step1900）。
