...
- **重複キー2分岐点の扱い — 裁定済み（2026-07-25、Gamba）**: 2件は別々の実分岐点で
  あり、seat 込み 4-tuple キーによる**分離扱い（485 点）で確定**。§1 の
  `aggregate_final.json` が第2枠の正（旧 3-tuple での併合は集計バグであり、
  本裁定により第2枠に未決事項なし。採取側も同一 game の異なる局からの抽出で
  プロトコル準拠 — 1局最大1分岐点の制約に抵触しない）。
- 実効枠（§5a-1b 適用後）揃い後の正式判定。第2枠の数値は本書 §1 を転記可。
