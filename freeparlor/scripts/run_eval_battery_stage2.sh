#!/usr/bin/env bash
# Stage2 完走後 eval バッテリー（レンズ1: 標準argmax、stage2_design.md §5）
# checkpoints: init / 2000(旧run stage2_20260709_092541) / 4000(旧run) /
#              8000 / 12000 / 16000 (resume run stage2_20260709_194510_resume)
# 自然分布 (p_enrich=0、eval 経路の既定)、seeds [10000, 10100)、100 半荘直列実行
set -euo pipefail

REPO="/home/gamba/mahjong/Mortal"
RUN_DIR="${RUN_DIR:-/home/gamba/mahjong/runs/ppo/stage2_20260709_194510_resume}"
OLD_RUN_DIR="/home/gamba/mahjong/runs/ppo/stage2_20260709_092541"
...
echo "=== Stage2 eval battery (lens1: standard argmax): $(date -Iseconds) ==="
...
LABELS=(
  [init]="$INIT_CKPT"
  [step2000]="$OLD_RUN_DIR/checkpoints/step_002000.pth"
  [step4000]="$OLD_RUN_DIR/checkpoints/step_004000.pth"
