...
export RUN_DIR="/home/gamba/mahjong/runs/ppo/l1_o3_${RUN_SUFFIX}"
export PPO_CONFIG="$REPO/freeparlor/configs/ppo_l1_o3.toml"
export CONFIG_TAG="l1_o3_${RUN_SUFFIX}"
export TMUX_SESSION="${TMUX_SESSION:-ppo_l1_o3_${RUN_SUFFIX}}"
...
PLACEHOLDER="l1_o3_PENDING_LAUNCH"
RESOLVED="l1_o3_${RUN_SUFFIX}"
...
# §11 が凍結した値を発進時に検査する（単一変数 = ppo_epochs のみ）
for kv in "ppo_epochs = 1" "submit_every = 50" "max_steps = 16000" "save_every = 2000"; do
  if ! grep -qF "$kv" "$PPO_CONFIG"; then
    echo "FATAL: expected '$kv' in $PPO_CONFIG (l1_o3_ppo_epochs_design.md §11)" >&2
    exit 1
  fi
done
if grep -qE '^ppo_epochs = 4$' "$PPO_CONFIG"; then
  echo "FATAL: $PPO_CONFIG still has ppo_epochs = 4 (O3 の介入になっていない)" >&2
  exit 1
fi
# O1 の介入を持ち込んでいないこと（1 branch = 1 variable）
if grep -qE '^submit_every = 10$' "$PPO_CONFIG"; then
  echo "FATAL: $PPO_CONFIG has submit_every = 10 (O1 の介入が混入 = 2 変数)" >&2
  exit 1
fi
...
echo "frozen config values verified: ppo_epochs=1 / submit_every=50 / max_steps=16000 / save_every=2000"
