...
差し戻し修正1 (監督側の独立再実行で判明): GameplayLoader の事後再構築による
...
列そのものを game_key 別に記録する。GameplayLoader は分岐点のメタデータ
(at_kyoku/shanten/at_turn/点況順位) 抽出にのみ使う (候補判定・報酬計算も
同様、既存どおり)。

差し戻し修正2 (2度目、監督側の独立再実行で判明): 記録を到着順の単一 FIFO
と仮定していたが、react_batch へのバッチ到着順は rayon::spawn の完了順に
左右され実行ごとに変動する (libriichi/src/agent/mortal.rs)。到着順に依存
しない (game_key, role, seq) キー (seq はスロット単位の決定論的連番、
mortal.rs の step_seqs) で記録するよう変更した (drca_common.py 参照)。

出力は jsonl (分岐点メタデータ、branch_role/branch_seq/mask_obs_digest で
台本上の位置を特定) + 選択された game_key ごとの台本 sidecar jsonl
(`<game_key>.script.jsonl`、各行 {"role":..., "seq":..., "digest":..., "action":...})。
"""
...
def main():
    ...
    with out_path.open('w', encoding='utf-8') as fout:
        for offset in range(args.max_seeds):
            ...
            # SAME shared dict, keyed by (game_key, role, seq) -- not
            # arrival order, which is not reproducible across runs
            # (rayon::spawn completion order in libriichi/src/agent/mortal.rs).
            script_store: dict[str, dict[str, dict[int, list[tuple[str, int]]]]] = {}
            wrapped_chal = RecordingPassthroughEngine(
                engine_chal, script_store, name='drca_collect_a_rec', role='challenger',
            )
            wrapped_champ = RecordingPassthroughEngine(
                engine_champ, script_store, name='drca_collect_b_rec', role='champion',
            )
            arena.py_vs_py(
            ...
            for split in SPLITS:
                ...
                game_store = script_store.get(game_key, {})
                if not game_store:
                    raise RuntimeError(
                ...
                for kyoku in kyoku_values:
                    ...
                    # semantics, used for metadata only) to its (role, seq)
                    # position in the true recorded query stream, by digest,
                    # searched across all role/seq buckets for this
                    # game_key. Exactly one match is required (differential
                    # fix item 1) -- 0 or 2+ is a loud FAIL, not silently
                    # resolved.
                    branch_role, branch_seq = find_unique_role_seq_match(game_store, digest)

                    script_path = log_dir / f'{game_key}.script.jsonl'
                    if not script_written:
                        write_script_sidecar(script_path, game_store)
                        script_written = True
                    ...
                    record = {
                        ...
                        'script_len': script_bucket_len(game_store),
                        'seat': seat,
                    ...
                    log.info(
                        ...
                        'shanten=%d n_candidates=%d branch_role=%s branch_seq=%d '
                        'script_len=%d score_rank=%d',
                        n_collected, args.n, game_key, seat, local_idx,
                        record['at_kyoku'], record['shanten'], len(candidates),
                        branch_role, branch_seq, record['script_len'], rank,
                    )
