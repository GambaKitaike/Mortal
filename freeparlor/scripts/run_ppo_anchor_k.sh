#!/usr/bin/env bash
# PPO Anchor Arm K — KL anchor to frozen init (anchored_ppo_design.md §3)
set -euo pipefail
REPO="/home/gamba/mahjong/Mortal"
RUN_SUFFIX="${RUN_SUFFIX:-$(date +%Y%m%d_%H%M%S)}"
export RUN_DIR="/home/gamba/mahjong/runs/ppo/anchor_k_${RUN_SUFFIX}"
export PPO_CONFIG="$REPO/freeparlor/configs/ppo_anchor_k.toml"
export CONFIG_TAG="anchor_k_${RUN_SUFFIX}"
export TMUX_SESSION="${TMUX_SESSION:-ppo_anchor_k_${RUN_SUFFIX}}"
