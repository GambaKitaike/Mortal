#!/usr/bin/env bash
# grp_baseline (DQN 時代の固定 baseline) 相手の 1v3 eval バッテリー — Stage3 版
# checkpoints: init / step16000 (stage3_20260712_033403)
# challenger: seed [10000, 10100) = 100 seed * 4 hanchan/seed = 400 半荘/checkpoint
# Stage1/2 (run_eval_grp_baseline_1v3.sh 系) と同一条件・同一 baseline
# stage3_design.md §6 レンズ2 (配備税チェック、事前登録 2SE 基準)
set -euo pipefail
...
# Stage1/2 と同一の baseline 選定 (2026-07-08 決定、grp_baseline.pth は配管 fixture のため使用禁止)
...
echo "=== Stage3 grp_baseline 1v3 eval battery: $(date -Iseconds) ==="
