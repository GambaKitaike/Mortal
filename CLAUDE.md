...
- **Stage3-16000 argmax の専門家定性レビューを記録（初回1半荘、判定非関与）**
  ...
  下手」は商品性最悪象限という暫定所見
- **教師データ非依存の訓練方式 候補棚卸しを起草（2026-07-25、docs-only・DRAFT・
  非事前登録・裁定非関与・進行中 run 無変更）**:
  `freeparlor/docs/design/teacherfree_training_candidates.md`。ブレスト（Gamba × 設計
  監督）の成果を、**問題 I（cold start = 天鳳教師フェーズの代替）と 問題 II（均衡脱出）
  の分離**という軸で整理。候補5本 — (1) シミュレータ由来の自己教師あり補助タスク
  （問題 I の最有力）、(2) リーグ訓練・敵対的搾取者（問題 II・最も安い・検証兼用 =
  0b 議題3 と同一物）、(3) KL 正則化ダイナミクス R-NaD 系（**不採用寄り** — 売りが
  理論保証であり4人戦で消える。L1/L2 でなく KL を使う理由＝裾感度・連鎖則・PPO 自体が
  KL 近似、は再利用可能な論点として §4b に保存）、(4) 信念状態つき探索（問題 II の
  本丸・最も高い。サンプリング絶滅をラダーの延命策と違い原理から解消。DRCA は
  この探索の手動1点版）、(5) HITL（当初案は不成立 = ラベル量の桁 + Stage3 の人力版。
  修正版は「人間は answer ではなく attention」）。**一次ソース調査で判明した重要事実**:
  (a) obs `version = 4` は既に単騎計算機（`algo/sp`）の出力を**入力**として持つ
  （打牌別有効牌 bitmap・巡目別の聴牌/和了確率・EV）ため、候補1 の marginal value は
  自手の事実ではなく**隠れ情報 oracle 層**にある（§2b）、(b) 観測用 SP 計算は
  `agent_helper.rs` で `calc_tegawari: false`/`calc_shanten_down: false` と
  **意図的に無効化**されており、Gamba の「シャンテン数・受け入れ枚数は指標として欠陥」
  指摘は「入力の忠実度」= この2フラグに着地する（候補1 より安い単一変数実験候補、§2d）、
  (c) oracle テンソル（相手の手牌・シャンテン・待ち、217×34）と `AuxNet` 前例は既存で
  Rust 変更ゼロで到達可能・全 call site が `is_oracle=False` で休眠（§2f）。
  `policy_session_0b_frame.md` に**議題5（次世代訓練方式の選択）を追加**（§0/§3 表/§4
  裁定順。§2 シナリオ節は無変更）、README 主要文書に1行追加。凍結条件
  （`drca_probe_design.md` §4/§5a/§7 の解禁条件）は一切無変更、判定条件を含まない
...
0b. **探索ラダー閉幕後の方針設計セッション**（設計監督側）: 立直マキシマリズムの
   ...
   **議題5（次世代訓練方式の選択）を追加（2026-07-25）、候補棚卸しは
   `teacherfree_training_candidates.md`（DRAFT・非事前登録 — 「現在の状態」節参照）。
   議題3（搾取者訓練）は同書の候補2 と同一物なので連続で扱う**。
   残り: 商品性要件の Gamba ヒアリング → DRCA 判定後にセッション実施（各候補の着手は
   個別の design md で事前登録してから）
