...
- **未確定（意図的）**: 次の軸。§6a の kl_beta=0.4 再走（許容・最大1回）の実施可否と、
  「C=IV かつ K=III」という事前登録が覆っていない組み合わせの扱いは **0b の裁定事項**。
  **次セッションで洗い出しと優先順位付けを行う → 申し送りは
  `freeparlor/docs/ops/session_handover_20260728.md`**
