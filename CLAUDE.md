...
0. **DRCA プローブ（診断、事前登録済み・実装未着手）**: ~~解釈条件の凍結~~
   **消化済み（2026-07-13、`drca_probe_design.md` §5a — 「現在の状態」節参照）**。
   残り: 実装（採取・並走・集計ハーネス3本）→ パイロット 50 分岐点 → 規模確定
   amendment（§5a-1 の規則適用のみ）→ 本測定 → 判定。
