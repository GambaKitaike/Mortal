...
- **DRCA no-call 腕の非鳴き制限サンプリング化 実装完了**（2026-07-14、実装エージェント
  ...
  つき検定内容は不変）。**残りはパイロット (50分岐点) → 規模確定 amendment →
  本測定 → 判定（別タスク、バックログ0）**

## 残タスク（バックログ、2026-07-14 時点）
...
0. **DRCA プローブ（診断、事前登録済み）**: ~~解釈条件の凍結~~ ~~実装（採取・並走・
   ...
   小修正、`drca_run_probe.py` のみ）~~ **全て消化済み（2026-07-14、「現在の状態」
   節参照。実装は smoke test のみ、パイロットは未実施）**。
   残り: パイロット 50 分岐点 → 規模確定 amendment（§5a-1 の規則適用のみ）→
   本測定 → 判定。7b「ギャップの支配項は立直ペイロード」読みの反実仮想による直接検証
