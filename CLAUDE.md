...
- **完走（2026-07-30 15:48）+ eval 中**: **`anchor_k_b04_20260729_150631`** —
  ...
  **完走確認済み**: `reached step 16000` の COMPLETED 経路 / diag max=16000 /
  ...
  兆候なし）。**eval バッテリーも完了**（07-30 18:35、EVAL_EXIT=0。argmax 6ckpt +
  1v3 n=800 両脚 + ミラー較正 **overall PASS**（理論値と dev=0.0000 で完全一致）+ メタ対決）。
  **測定記録は `anchor_k_b04_measurements_20260730.md`（判定は未起草 — レンズ4 が先）**:
  判定1 の測定器 = 放銃 11.85%→13.00%（**z=+2.21**、閾値 2 を跨がない）、
  判定2 = チップ +0.035→+0.440（差 **+0.405 = +1.75SE**）⇒ **機械的には象限 III（K と同じ）**。
  **β×4 は判定を動かさなかった**（放銃 z +2.51→+2.21、チップ +2.18SE→+1.75SE。
  **どちらも run 間ばらつきより小さい変化**なので「β=0.4 では足りない」と読むのは過剰）。
  **C/K と像が違う点2つ**: (a) **ダマ和了が消滅していない**（init 21.30% → **b04 12.41%**、
  C 0.82% / K 0.80%）= **立直マキシマリズムが初めて部分的に止まった**。立直率も β に単調
  （init 24.27% → b04 30.95% → K 35.92% → C 37.34%）、降りの中断率も単調
  （init 23.78% → b04 27.48% → K 33.75% → C 49.29%）。(b) 打点は**層内でも有意に動いた**
  （立直和了 +540 z=+2.74 / ダマ和了 **−1253 z=−5.07**。C/K は全層 n.s. だった）ので
  「構成シフト由来だから引用不可」の留保は b04 では**分けて書く**必要がある。
  **β を上げて失った/悪化した資産**: 加カン消滅（0.00%、z=−3.49。K では 0.10% 保存）、
  孤立牌の切り順逆転率が有意に劣化（z=+2.27。K は n.s.）。
  **鳴き判断は K と同程度に悪いまま**（有役テンパイ機会の見送り 91.16% vs K 90.51%）＝
  0b 議題1 の再検討条件は未達。**牌効率は2指標が逆を向いた**（向聴を外した率 0.72%→0.55%
  **z=−3.25 で有意に改善**＝3 arm 初 / 受け入れで支配された率 14.74%→15.78% z=+4.14 で劣化）。
  補助診断の生出力は run dir の `logs/aux_diagnostics_20260730/` に保全。
  **レンズ4 実施済み（2026-07-31、Gamba・自己対戦1半荘。
  `qualitative_review_anchor_k_b04_20260731.md`）** — 所見15件を全件定量に突き合わせた。
  **最大の収穫: ダマ和了の復活（12.41%）は改善ではない**（該当2件は「リーチすべき場面」と
  評価。ダマ打点も −1253点 z=−5.07）＝ **行動分布の記述としては正しいが質の改善として
  引用してはならない**。チーテン取らず3件・牌効率のミス3件はいずれも定量と一致。
  新規実装した**暗刻からのポン率**（`analyze_ankou_pon.py`）で「暗刻から南ポン・直らないね」を
  測定 → init 0.97% / b04 1.86%（z=+1.83 n.s.、4枚目は 20/20 で同局に切る）＝
  **init から継承した欠陥で PPO では直らない**。起票7件。
  次: **判定の起草 → §9 へ追記**
- **新規（2026-07-31、レンズ4 の副産物・裁定待ち）**: **libriichi は天鳳準拠で西入
  （サドンデス）を実装しているが、フリー雀荘ルールでは通常採用しない**という不一致を発見
  （`freeparlor/docs/design/parlor_rule_west_round_design.md`。実装は `arena/game.rs:79-86`）。
  実測（`analyze_west_round_impact.py`）: 西入は **半荘の 3.38%（init）/ 3.62%（b04）**で発生し、
  そのうち **48–74% で順位点が変わる**。全半荘に均した寄与は 順位点 −0.013（init）/
  **−0.113（b04）**、チップ −0.001 / −0.005 枚 ⇒ **現行の凍結済み判定はどれも動かない**
  （最大でも順位点で 1SE の 8%）。ただし事後集計なので**オーラスの意思決定変化は含まない**。
  選択肢は W1 config フラグ化（既定=現状でビット不変・推奨）/ W2 ハード変更 / W3 据え置き。
  **実施順が要注意**: O1 は**西入ありで訓練された** Stage1-16000 を baseline に凍結済みなので、
  **O1 の前にルールを変えると submit_every と2変数になる**（S1 = O1 を先に走らせる、を推奨）
