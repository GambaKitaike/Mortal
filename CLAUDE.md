...
- **現在の優先軸**: **未確定（次の軸は Gamba 裁定待ち）**。候補は
  (a) **候補1（oracle 蒸留）** — 0b 裁定 §2 の順序上は次 /
  (b) **`ppo_epochs=1` で BR を計算し直す**（候補2 の再走ではなく新実験。
  O3 は劣化が系列最小だったので「手続きを直せば BR 演算になるか」を直接試せる）/
  (c) レンズ4 起票の計装（GPU 不要・現在6件）/
  (d) `ppo_epochs=1` 既定化のための 2 seed 目
