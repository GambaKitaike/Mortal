...
**L1（最適化衛生）系列（O1 / O3）は 2026-08-02 に閉幕**（総括は
`freeparlor/docs/reports/l1_series_summary_20260802.md`）。
**候補2（敵対的搾取者）系列も 2026-08-04 に閉幕**（2 run とも測定不能。総括は
`freeparlor/docs/reports/exploiter_series_summary_20260804.md`）。次の軸は Gamba 裁定待ち。

**最初に読む文書（この順で）:**
1. `freeparlor/docs/design/ppo_migration_design.md` — PPO 移行の設計正典
2. `freeparlor/docs/reports/anchor_series_summary_20260731.md` — **anchor 系列の総括
   （閉幕）**。基礎技能とEVの乖離という本プロジェクトの中心的知見
2b. `freeparlor/docs/design/l1_o1_submit_every_design.md` — **現行軸（L1 O1）の事前登録**（凍結済み）
...
- **閉幕（2026-08-02、Gamba 裁定）**: **L1（最適化衛生）系列は O1 / O3 とも判定1（基礎維持）が
  ...
  **中心的知見: clip_fraction を支配していたのは staleness ではなく
  ...
  0b 議題1 の否決条件は未充足）
- **閉幕（2026-08-04、Gamba 裁定）**: **候補2（敵対的搾取者）系列は 2 run とも
  「測定不能」で終了**。総括は `freeparlor/docs/reports/exploiter_series_summary_20260804.md`。
  **中心的知見: この PPO 設定は固定した強い相手に対する
  「ベストレスポンス演算子」として機能していない**。初期値を mortal init にしても
  標的のコピーにしても、**標的に対する成績が訓練前より下がった**（−2.96 / −3.52 千点/半荘）。
  2本目は初期値 = 標的なので step0 の A が構成上 0 で、そこから
  **0 → −0.275 → −1.512 → −1.062 → −3.562 → −3.519 と単調に悪化**した
  （§3-0 の門 `A′ > C` が −6.38SE で不成立）。
  ⇒ 失敗の原因は初期値ではなく**訓練手続きそのもの**。
  **これは全 stage で観測してきた基礎技能劣化が、チップ獲得という埋め合わせ無しに
  裸で出たもの**（搾取者2 はチップが本プロジェクト初のマイナス −0.017 枚/局）。
  **anchor 総括の留保1 は未解消**だが、理由が「未実施」から
  「**現行手続きでは測れない**」へ変わった
- **事前登録の穴（申し送り）**: 「搾取者は弱くてよい」とだけ登録し、
  **DiD が出ず かつ 搾取者が弱い**場合の読み方が無かった。次回は
  **BR 演算の成功条件（最低でも A > C）を主判定と別に事前登録**し、
  満たさない run は「搾取なし」ではなく「測定不能」と判定する
- **本リポジトリでの次の軸（2026-08-04 Gamba 裁定）**: **候補1（oracle 蒸留）は
  本リポジトリでは実施しない**。純粋な自己教師あり学習を目指す方向で
  **Mortal とは完全に別物になる**ため、**別リポジトリを新設して進める**。
  ⇒ 本リポジトリに残る候補は
  (a) **`ppo_epochs=1` で BR を計算し直す**（候補2 の再走ではなく新実験。
  O3 は劣化が系列最小だったので「手続きを直せば BR 演算になるか」を直接試せる。
  `exploiter_series_summary_20260804.md` §6）/
  (b) レンズ4 起票の計装（GPU 不要・現在6件）/
  (c) `ppo_epochs=1` 既定化のための 2 seed 目（L1 総括 §6-1 の未決裁定）/
  (d) L1 の残り O2（判定窓を消費半荘数で定義し直す必要あり）
...
### 閉幕: anchor 系列（2026-07-31、Gamba 裁定）

> **総括は `freeparlor/docs/reports/anchor_series_summary_20260731.md` が正。**
> C / K / b04 の run 経緯・eval 数値・レンズ4 の詳細は
> `freeparlor/docs/ops/project_history.md` の末尾へ不改変で移設した。

- **3 arm すべてで判定1（基礎維持）は不成立**。アンカーでは基礎劣化を止められなかった
- **判定2（経済適応）は KL 側の2 arm で成立**（K +2.18SE / b04 +1.75SE）。pool 側 C は +0.45SE で不成立
  ⇒ **引き戻しは相手分布（pool）より損失側（KL）に置くほうが効く**（単一変数で確定）
- **β を 0.1→0.4 にしても象限は動かなかった**（放銃 z +2.51→+2.21 / チップ +2.18→+1.75SE）。
  `kl_ref_mean` は 0.13048→0.06895 と半減しており β は機構としては効いている。
  変化幅は run 間ばらつきより小さい ⇒ **「β が足りない」ではなく「この設計では跨げない」**
- **§6a の再走規定は使い切った**ので系列はここで閉じる（`anchored_ppo_design.md` は closed）
- **総括の中心（Gamba 裁定）**: **基礎技能は悪化したが EV は大きく上昇した**
  （チップ +0.4〜0.5枚/半荘 = +2.0〜2.5千点相当、合算の点推定 +3.4〜+3.8千点/半荘。
  ただし**有意なのはチップ単独**で、合算は 1.2〜1.4SE）。
  ⇒ **チップありのルールでは基礎技能（放銃率・和了率）だけで AI の性能を説明できない**
  （放銃が有意に増え和了も減っているのに avg_rank は不変・収支は改善した）。
  **留保4件**: 相手は init 1種で搾取可能性は未検証（= 0b 議題3 の敵対的搾取者が検定になる）/
  合算は有意水準未達 / **EV 上昇は plain PPO 由来でアンカーが作ったものではない**
  （Stage1 で既にチップ +0.520。アンカーの寄与は「獲得を保ったまま劣化を 1/3 に圧縮」）/
  商用採否は 0b 議題1 で否決のまま（鳴き判断が init 水準に戻っていない）
...
9. ~~**anchor 系列**~~ **消化済み・閉幕（2026-07-31、Gamba 裁定）**: C（象限 IV）/
   K（象限 III）/ b04（象限 III、§6a の再走を使い切り）で終了。総括は
   `freeparlor/docs/reports/anchor_series_summary_20260731.md`、
   設計書 `anchored_ppo_design.md` は **closed**（結果は §9 に追記済み）。
   持ち越すのは **順序関係の知見**（引き戻しは pool より損失側）と**配管**
   （ref forward / pool の anchor 分岐 / 検定19・20 / 判定書式 n=800）。
   L2 の次の一手は **D3（残差方策）**で、Arm K の ref 配線を再利用できる（未登録）
9b. **西入（サドンデス）の除去 = W1**（`freeparlor/docs/design/parlor_rule_west_round_design.md`、
   **2026-07-31 Gamba 裁定 = W1 + S1**）: libriichi は天鳳準拠で西入を実装しているが
   フリー雀荘ルールでは通常採用しない（`arena/game.rs:79-86`）。
