...
1. **Stage3 実装**（設計確定済み・実装待ち）: ボーナス配線（訓練 client のみ、
   eval 側 0 assert）+ 検定追加（b=0 ゴールデンハッシュ / b>0 合成ケース /
   eval dump assert）+ `ppo_stage3.toml` + `run_ppo_stage3.sh`。仕様は
   `stage3_design.md` §2/§7 が正
1b. **Stage3 発進前提の drain 清掃**（`stage3_design.md` §8）: Stage2 完走 run の
   `drain/` 削除（Gamba 承認必須）+ preflight へのディスク検査追加
