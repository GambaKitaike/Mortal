...
- **閉幕（2026-08-02、Gamba 裁定）**: **L1（最適化衛生）系列は O1 / O3 とも判定1（基礎維持）が
  ...
  **中心的知見: clip_fraction を支配していたのは staleness ではなく
  ...
  0b 議題1 の否決条件は未充足）
- **判定済み（2026-08-04）**: **候補2（敵対的搾取者）は2本とも「測定不能」**。
  1本目 = mortal init 初期化（`exploiter_verdict_20260803.md`）、
  2本目 = 標的コピー初期化（`exploiter2_result_verdict_20260804.md`）。
  **2本目が決定的**: 初期値 = 標的なので step0 の A は構成上 0（ミラー）だったが、
  **0 → −0.275 → −1.512 → −1.062 → −3.562 → −3.519 と単調に悪化**した。
  §3-0 の成功条件 `A′ > C` は −6.38SE で不成立 ⇒ 主判定を出さず測定不能。
  ⇒ **2 run 揃って「標的相手に訓練前より弱くなった」**（1本目 +1.444→−1.512 /
  2本目 0→−3.519）ので、**失敗の原因は初期値ではなく訓練手続き**と診断できる。
  1本目の判定 §4 が立てた仮説（init から立ち上がらないのが原因）は**否定された**。
  **この PPO 設定は固定した強い相手に対する BR 演算子として機能しない**。
  anchor 総括の留保1 は未解消のまま（ただし理由が「未実施」から
  「**現行手続きでは測れない**」へ変わった）。**候補2 は閉幕を推奨（裁定待ち）**
