...
- **閉幕（2026-08-02、Gamba 裁定）**: **L1（最適化衛生）系列は O1 / O3 とも判定1（基礎維持）が
  ...
  **中心的知見: clip_fraction を支配していたのは staleness ではなく
  ...
  0b 議題1 の否決条件は未充足）
- **判定済み（2026-08-03）**: **候補2（敵対的搾取者）1本目は「測定不能」で終了**
  （判定 `freeparlor/docs/reports/exploiter_verdict_20260803.md`）。
  主判定（順位点 EV の DiD ≥ +2SE）は **+0.12SE で不成立**だが、
  **この run 自体がベストレスポンス演算として失敗している**（標的相手に init より弱い
  A −1.512 < C +1.444 / 向聴を外した率が init の6倍 / チップも稼がず）ため、
  **「立直マキシマリズムは搾取不能」とは結論できない**。
  anchor 総括の留保1（搾取可能性は未検証）は**未解消のまま残る**。
  ⇒ **候補2 は閉幕せず、2本目（標的のコピーから初期化）へ**。
  これは `teacherfree_training_candidates.md` §3e が事前登録済みの分岐
...
- **現在の優先軸**: **未確定（次の軸は Gamba 裁定待ち）**。候補は
  (a) **搾取者2本目**（標的のコピー初期化。上記の事前登録済み分岐）/
  (b) **候補1（oracle 蒸留）** — 0b 裁定 §2 の順序上は次 /
