...
7. ~~drain 清掃の恒久運用~~ **明文化済み（2026-07-16、
   `freeparlor/docs/ops/run_artifact_retention.md` — 判定 commit 後のイベント駆動
   清掃 + 許可リスト方式 + 成果物クラス別保持ポリシー）**。残り: 同書 §5 の清掃候補
   3件（stage3 drain 368GB ほか、計 ~412GB）の **Gamba 承認 → 削除実施**、および
   `buffer/`（.traj、stage2_resume に 36GB）の用途調査
